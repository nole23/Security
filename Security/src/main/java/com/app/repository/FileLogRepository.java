...
public interface FileLogRepository extends JpaRepository<FileLog, Long> {
	 ...
	 */
	@Query("SELECT f from FileLog as f WHERE (f.message LIKE %:message%) OR (f.source LIKE %:source%) OR (f.message LIKE %:message%) OR (f.time =  :time) OR (f.serverTime = :serverTime) OR (f.date =  :date) OR (f.agentId LIKE %:agentId%) OR (f.count = :count) OR (f.tagName LIKE %:tagName%) OR (f.logLevel = :logLevel) OR (f.platform LIKE %:platform%)")
